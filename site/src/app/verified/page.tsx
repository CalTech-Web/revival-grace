...
import { CheckCircle, AlertTriangle, Loader2, Clock, Mail } from "lucide-react";
...
export default function VerifiedPage() {
  ...
  const [status, setStatus] = useState<"form" | "loading" | "verified" | "pending" | "not_found" | "error">("form");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/check-subscriber?email=${encodeURIComponent(email.trim())}`);
      if (!res.ok) {
        setStatus("error");
        return;
      }
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center">
        {status === "form" && (
          <div className="py-12">
            <Mail className="w-16 h-16 text-primary mx-auto mb-4" strokeWidth={1.5} />
            <h1 className="font-serif text-[28px] font-bold text-heading mb-2">
              Confirm Your Subscription
            </h1>
            <p className="text-[14px] text-text mb-6 leading-relaxed">
              Enter the email address you subscribed with to unlock all devotionals.
            </p>
            <form onSubmit={handleSubmit} className="space-y-4 max-w-sm mx-auto">
              ...
              <button
                type="submit"
                className="w-full py-3 bg-primary text-white text-[13px] font-medium tracking-wide rounded-md hover:bg-primary-dark transition-colors"
              >
                Verify & Continue
              </button>
            </form>
        ...
        {status === "pending" && (
          <div className="py-12">
            ...
            <p className="text-[14px] text-text mb-6">
              Your email has not been confirmed yet. Please check your inbox and click the confirmation link first.
            </p>
            <button
        ...
        {status === "not_found" && (
          <div className="py-12">
            ...
            <p className="text-[14px] text-text mb-6">
              We could not find a subscription with this email. Please subscribe first to get access.
            </p>
            <button
